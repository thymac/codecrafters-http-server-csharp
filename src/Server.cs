...
class Program
{
  ...
  private static readonly ConcurrentDictionary<Socket, byte> activeSockets = new ConcurrentDictionary<Socket, byte>();
  ...
  static async Task Main(string[] args)
  {
    ...
    {
      server.Start();
      logger.LogInformation("Server started on {IpAddress}:{Port}", IPAddress.Any, PORT);
      while (true)
      {
        var socket = await server.AcceptSocketAsync();
    ...
    {
      logger.LogError(ex, "An error occurred in the server.");
    }
    finally
    {
      server.Stop();
  ...
  static async Task HandleSocketAsync(Socket socket)
  {
    activeSockets.TryAdd(socket, 0);
    try
    {
      logger.LogInformation("New connection from {RemoteEndPoint}", socket.RemoteEndPoint);
      var buffer = new byte[BUFFER_SIZE];
      var n = await socket.ReceiveAsync(buffer, SocketFlags.None);
      var request = Encoding.UTF8.GetString(buffer, 0, n);
      var httpContext = HttpContext.Parse(request);

      Func<HttpContext, Task<byte[]>> handler = httpContext switch
      {
        { Method: "GET", Path: "/" } => HandleHome,
        { Method: "GET", Path: var path } when path.StartsWith("/files/") => HandleGetFile,
        { Method: "POST", Path: var path } when path.StartsWith("/files/") => HandlePostFile,
      ...
      logger.LogInformation("{RemoteEndPoint} requests {Path} responded with {Response}",
                            socket.RemoteEndPoint, httpContext.Path,
                            Encoding.UTF8.GetString(response.AsSpan(0, Math.Min(response.Length, 100))));

      await socket.SendAsync(response, SocketFlags.None);
    }
    catch (Exception ex)
    {
      logger.LogError(ex, "An error occurred while handling the connection from {RemoteEndPoint}", socket.RemoteEndPoint);
    }
    finally
    {
      socket.Shutdown(SocketShutdown.Both);
      socket.Close();
      activeSockets.TryRemove(socket, out _);
    }
  }

  static Task<byte[]> HandleHome(HttpContext httpContext) => Task.FromResult(okHttpResponse);

  static Task<byte[]> HandleDefault(HttpContext httpContext) => Task.FromResult(notFoundHttpResponse);

  static async Task<byte[]> HandleEcho(HttpContext httpContext)
  {
    var value = httpContext.Path.Split('/', StringSplitOptions.RemoveEmptyEntries)[1];
    var encodingHeader = string.Empty;
    byte[] responseBody;
  ...
  static Task<byte[]> HandleUserAgent(HttpContext httpContext)
  {
    var userAgent = httpContext.Headers.TryGetValue("User-Agent", out var userAgentValue) ? userAgentValue : string.Empty;
    var response = $"HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: {userAgent.Length}\r\n\r\n{userAgent}";
  ...
  static async Task<byte[]> HandleGetFile(HttpContext httpContext)
  {
    var fileName = httpContext.Path.Split('/', StringSplitOptions.RemoveEmptyEntries)[1];
    var absolutePath = Path.GetFullPath(Path.Combine(directory, fileName));

    if (!absolutePath.StartsWith(directory) || !File.Exists(absolutePath))
    {
      return notFoundHttpResponse;
    }
    ...
    try
    {
      ...
      return Encoding.UTF8.GetBytes(response).Concat(content).ToArray();
    }
    catch (Exception ex)
    {
  ...
  static async Task<byte[]> HandlePostFile(HttpContext httpContext)
  {
    var fileName = httpContext.Path.Split('/', StringSplitOptions.RemoveEmptyEntries)[1];
    var absolutePath = Path.GetFullPath(Path.Combine(directory, fileName));
    ...
    try
    {
      await File.WriteAllTextAsync(absolutePath, httpContext.Body);
      return createdHttpResponse;
    }
    catch (Exception ex)
    {
      logger.LogError(ex, "Error writing file: {FileName}", fileName);
      return notFoundHttpResponse;
    }
  }
